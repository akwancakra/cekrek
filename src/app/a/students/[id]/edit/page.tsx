...
export default function EditStudentPage() {
    ...
    const handleNextStage = () => {
        if (currentStage !== 5) {
            ...
            if (nextStageIndex <= stages.length) {
                ...
                router.push(
                    `/a/students/${id}/edit?stage=${stages[nextStageIndex - 1]}`
                );
            }
        }
    };
    ...
    const handleStageClick = (stageIndex: number) => {
        if (validateStage(stageIndex - 1)) {
            ...
            router.push(
                `/a/students/${id}/edit?stage=${stages[stageIndex - 1]}`
            );
        } else {
    ...
    useEffect(() => {
        ...
        router.replace(
            `/a/students/${id}/edit?stage=${stages[stageToSet - 1]}`
        );
    }, [id]);
