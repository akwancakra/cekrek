...
import BiodataWrapper from "@/components/elements/child-add/parent/BiodataWrapper";
import PreviewData from "@/components/elements/child-add/parent/PreviewData";
...
export default function AddStudentPage() {
    ...
    const handleNextStage = () => {
        if (currentStage !== 5) {
            ...
            if (nextStageIndex <= stages.length) {
                ...
                router.push(
                    `/p/childs/add?stage=${stages[nextStageIndex - 1]}`
                );
            }
        }
    };
    ...
    const handleBackStage = () => {
        if (currentStage !== 1) {
            ...
            if (previousStageIndex > 0) {
                ...
                router.push(
                    `/p/childs/add?stage=${stages[previousStageIndex - 1]}`
                );
            }
        }
    };
    ...
    const handleStageClick = (stageIndex: number) => {
        if (validateStage(stageIndex - 1)) {
            ...
            router.push(`/p/childs/add?stage=${stages[stageIndex - 1]}`);
        } else {
    ...
    useEffect(() => {
        ...
        router.replace(`/p/childs/add?stage=${stages[stageToSet - 1]}`);
    }, []);
...
"
                ...
                <div className="sm:flex group-[.open]:block md:group-[.open]:flex">
                    ...
                    <div className="sticky top-4 rounded-lg p-2 bg-white border border-gray-300 w-full h-fit sm:w-1/3 mt-3 sm:mt-0 group-[.open]:mt-3 md:group-[.open]:mt-0 group-[.open]:w-full md:group-[.open]:w-1/3 dark:bg-neutral-800 dark:border-neutral-600">
                        <div className="max-w-32 mb-3 bg-gray-300 border border-gray-300 rounded-lg overflow-hidden dark:border-neutral-700">
