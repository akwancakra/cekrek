...
export default function EditChildParentPage() {
    ...
    const {
        ...
        isReady && profile?.id && `/api/parents/${profile?.id}/children/${id}`,
        fetcher
    );
    ...
    const handleNextStage = () => {
        if (currentStage !== 5) {
            ...
            if (nextStageIndex <= stages.length) {
                ...
                router.push(
                    `/p/childs/${id}/edit?stage=${stages[nextStageIndex - 1]}`
                );
            }
        }
    };
    ...
    const handleStageClick = (stageIndex: number) => {
        if (validateStage(stageIndex - 1)) {
            ...
            router.push(`/p/childs/${id}/edit?stage=${stages[stageIndex - 1]}`);
        } else {
    ...
    useEffect(() => {
        ...
        router.replace(`/p/childs/${id}/edit?stage=${stages[stageToSet - 1]}`);
    }, [id]);
